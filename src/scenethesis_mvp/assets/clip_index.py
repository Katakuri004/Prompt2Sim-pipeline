    ...
    def retrieve(
        ...
        with torch.no_grad():
            for obj in updated.objects:
                ...
                best_index = max(candidate_indices, key=lambda idx: float(scores[idx]))
                ...
                asset = registry.get(asset_ids[best_index])
                ...
                retrieval_records.append(
                    {
                        ...
                        "image_score": round(float(image_scores[best_index]), 6),
                        "text_score": round(float(text_scores[best_index]), 6),
                        "metadata_score": round(float(metadata_scores[best_index]), 6),
                    }
                )
        write_json(Path(out_dir) / "clip_retrieval.json", retrieval_records)
